    ...
    config:
      ...
      scrape_configs:
        - job_name: otel-collector-binary
          static_configs:
            - targets:
              # - localhost:8888 # Replace the target endpoint with a Prometheus exporter
...
<Admonition type="info">
  You can learn more about the configuration at [OTel Collector
  Configuration](https://signoz.io/docs/opentelemetry-collection-agents/opentelemetry-collector/configuration/#otel-collector-configuration-structure)
  doc.
</Admonition>
...
For a persistent Windows Service, you can create a Windows Service, you can create a service using `sc` command.
...
If running via systemd, check the logs:
...
- If you are collecting logs from files (not covered in the basic config above, but a common use case), ensure the `otelcol-contrib` user has read access to those files.
