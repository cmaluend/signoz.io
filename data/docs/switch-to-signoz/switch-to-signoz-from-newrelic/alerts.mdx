...
date: 2025-05-12
id: alerts
title: Switch Alerts from New Relic
description: Switching alerts from New Relic to SigNoz. How to switch alert configurations from New Relic to SigNoz.
---

Switching alerts from New Relic to SigNoz involves recreating your alert conditions and notification channels in the SigNoz platform. This guide will help you transition your alerting setup effectively.

## Understanding Alert Differences

Before switching, it's important to understand the key differences between New Relic and SigNoz alerts:
...
Before switching alerts, you'll need to set up notification channels in SigNoz. Both New Relic and SigNoz support various notification channels, though there are some differences in the available integrations.
...
If you manage alert rules as code in New Relic (for example, using `newrelic` Terraform provider), you can continue using the same approach when switching to SigNoz by utilizing the [SigNoz Terraform Provider](/docs/alerts-management/terraform-provider-signoz/).

## Verifying Alert Switching

After switching your alerts to SigNoz, verify that they're working correctly:
...
4. Gradually switch all alerts, starting with non-critical ones

## Next Steps

Once your alerts are successfully switched to SigNoz, consider:

- [Switching dashboards](/docs/switching/switch-from-newrelic/dashboards) to visualize metrics related to your alerts
