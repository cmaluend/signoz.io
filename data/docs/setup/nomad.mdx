...
driver](https://www.nomadproject.io/docs/drivers/docker). For background on the
pack itself, see the upstream [SigNoz Nomad Pack](https://github.com/hashicorp/nomad-pack-community-registry/blob/main/packs/signoz/README.md),
and review HashiCorp's [Nomad Pack guide](https://developer.hashicorp.com/nomad/tools/nomad-pack) if you need help installing or operating the `nomad-pack`
CLI.
...
There's a `setup.sh` script in the <a href="https://github.com/hashicorp/nomad-pack-community-registry/blob/main/packs/signoz/setup.sh" target="_blank" rel="noopener noreferrer nofollow">Pack directory</a> 
that can automatically perform the required setup steps. Set the `RELEASE`, `NAMESPACE`, and `CLICKHOUSE_PASSWORD` environment variables and it will make the appropriate
changes. If you prefer to run the same steps manually, follow the sequence below.
...
Ensure the SigNoz pack from the community registry is available (or refreshed to
the latest version) before you deploy:
...
Whenever you need to pull updates, rerun the `registry add` command above to
refresh the cached registry and then redeploy the pack.
...
If a job under this pack fails due to a "Failed Validation" error for
ClickHouse variables (this can occur if ClickHouse is not yet available and the
variables do not exist), it automatically retries. If all retry attempts are
exhausted, check the job status using `nomad job status <job-name>` or in the
Nomad UI, then re-run the job manually.
</Admonition>
...
1. Use `nomad job status <your-release-name>_signoz` (and the other jobs listed
   earlier) to ensure each job reaches the `running` state. Investigate any
   allocations stuck in `pending` or `dead` with `nomad alloc status
   <allocation-id>`.
2. Confirm that ClickHouse, ZooKeeper, and SigNoz appear in Consul with healthy
   checks before routing traffic through them.
3. After the `signoz` job reports healthy allocations, access the UI via the
   Consul service endpoint or by forwarding port `8080` and opening
   `http://<sigNoz-host>:8080/` in your browser.
