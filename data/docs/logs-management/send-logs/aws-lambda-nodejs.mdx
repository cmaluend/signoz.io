...
4.  Enter the ARN for your region and architecture from [OpenTelemetry Lambda Layers](https://github.com/open-telemetry/opentelemetry-lambda/releases).
...
By default, the OpenTelemetry layer exports to AWS backends. To send logs to SigNoz and enable the `telemetryapi` receiver (which captures Lambda logs), you must provide a custom configuration.

Go to AWS Lambda Console and navigate to your function. Create a file named `collector.yaml` in the root of your function code using the **Code** tab:
...
<Admonition type="note">
  Ensure this file is included in your deployment package (root directory).
</Admonition>
...
3.  Select **Specify an ARN**.
4.  Enter the ARN from [OpenTelemetry Collector Lambda Layers](https://github.com/open-telemetry/opentelemetry-lambda/releases), look for the `layer-collector` release.
...
| ------------------------------------ | --------------------------------------------- | ---------------------------------------------------------------- |
| `OPENTELEMETRY_COLLECTOR_CONFIG_URI` | `/var/task/collector.yaml`                    | **Crucial**: Points to your custom config file in the task root. |
| `OTEL_EXPORTER_OTLP_ENDPOINT`        | `https://ingest.<region>.signoz.cloud:443`    | Your SigNoz OTLP endpoint. Replace `<region>`.                   |
| `OTEL_EXPORTER_OTLP_HEADERS`         | `signoz-ingestion-key=<SIGNOZ_INGESTION_KEY>` | Your ingestion key.                                              |
