...
3. In the Network tab, look for the `query_range` request whose `request_type` is `raw` (if multiple `query_range` requests are available, choose the one with `request_type=raw`)
4. Right-click the `query_range` request → Copy → Copy as cURL
5. Paste the cURL command in your terminal
6. Before running, increase the `limit` value in the request to retrieve more records (see [Pagination in Log Search](https://signoz.io/docs/logs-management/logs-api/search-logs/#pagination-in-log-search))
7. Run the command to get the data in JSON format
