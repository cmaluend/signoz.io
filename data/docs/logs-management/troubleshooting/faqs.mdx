...
### Q. Why does my log download only include records visible on screen?
A. The download feature only exports logs currently visible in the UI. Use the SigNoz API for larger downloads.

<details>
<summary>Show detailed steps</summary>

The download feature in SigNoz only exports logs that are currently visible in the UI. This means you need to scroll through the logs to load them before downloading, which limits the amount of data you can export through the interface.

To download a larger dataset of logs, you can use the SigNoz API instead:

1. Open your browser's developer tools and go to the Network tab
2. Perform a log search in the SigNoz UI
3. Copy the `query_range` request from the network tab
...
5. Execute the API call to get the data in JSON format
...
[def]: https://opentelemetry.io/docs/languages/java/configuration/#agent-system-properties