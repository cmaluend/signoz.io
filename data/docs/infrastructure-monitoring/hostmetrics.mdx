...
### Host showing as blank/empty

**Symptoms:** Host name is blank/empty in Infrastructure Monitoring > Hosts tab

**Cause:** Metrics are missing the `host.name` resource attribute. This happens when the [resourcedetection processor](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/resourcedetectionprocessor/README.md) is not configured in your collector pipeline.

**Resolution:**

1. Add the `resourcedetection` processor to your `processors` section:
...
2. Ensure the processor is included in your metrics pipeline (between the receiver and exporter):
...
This processor automatically detects resource information from the host environment. This processor can detect attributes like hostname, OS information, cloud provider details, and Kubernetes metadata.
The `timeout` parameter controls how long the processor waits for detection to complete.
