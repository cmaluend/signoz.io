...
### Host showing as unknown (missing hostname) {#host-showing-as-unknown}

**Symptoms:** Host appears in Infrastructure Monitoring > Hosts tab with a dash (-) instead of a hostname, or hostname column shows as unknown.

**Cause:** Metrics are missing the `host.name` resource attribute. This happens when the `resourcedetection` processor is not configured in your collector pipeline.
...
The `resourcedetection` processor automatically adds resource attributes like hostname and OS information to your metrics. Without it, SigNoz cannot identify individual hosts.
