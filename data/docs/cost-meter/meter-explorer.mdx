...
### How do I separate environments in the Volume breakdown dashboard?

If your environments are not appearing separately in the "Volume breakdown by environment" dashboard, you need to add the `deployment.environment` attribute to your resource metadata in the OpenTelemetry Collector configuration.
...
<Admonition type="info">
  **Note on Attribute Naming**: You may notice that metrics will show the attribute as `deployment_environment` (with underscore) while logs will show `deployment.environment` (with dot). This is expected behavior due to current normalization in SigNoz.
</Admonition>

#### Additional Resources

For more detailed information on configuring resource attributes:
