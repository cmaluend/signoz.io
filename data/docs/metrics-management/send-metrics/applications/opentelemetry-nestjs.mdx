...
<KeyPointCallout title="About the export interval" defaultCollapsed={true}>
The export interval is configurable via the `OTEL_METRIC_EXPORT_INTERVAL` environment variable (in milliseconds). The default is 60 seconds, which reduces network overhead and is suitable for production. For development, you can set a shorter interval (e.g., `10000` for 10 seconds) for faster feedback. Shorter intervals provide more real-time visibility but increase network traffic and storage costs.
</KeyPointCallout>
