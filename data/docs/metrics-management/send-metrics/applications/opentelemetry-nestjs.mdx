...
export class MetricsService implements OnModuleInit {
  ...
  onModuleInit() {
    // HTTP metrics
    this.httpRequestCounter = this.meter.createCounter('http.requests.total', {
    ...
    this.httpActiveRequests = this.meter.createUpDownCounter('http.requests.active', {
    ...
    // Observable gauge for queue size
...
Create an interceptor to automatically track HTTP request metrics:

```typescript:src/metrics/metrics.interceptor.ts
import {
  Injectable,
  NestInterceptor,
  ExecutionContext,
...
export class MetricsInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        this.recordMetrics(method, url, response.statusCode, duration);
      }),
      catchError((error) => {
        const duration = Date.now() - startTime;
  ...
  private recordMetrics(
    ...
    this.metricsService.httpRequestCounter.add(1, attributes);
    this.metricsService.httpRequestDuration.record(duration, attributes);
    this.metricsService.httpActiveRequests.add(-1);
  }
}
```
...
Use the MetricsService in your business logic:

```typescript:src/orders/orders.service.ts
...
export class OrdersService {
  ...
  async createOrder(orderData: CreateOrderDto) {
    ...
    // Record metrics
    this.metricsService.ordersCreated.add(1, {
      region: orderData.region,
    });

    this.metricsService.orderValue.record(total, {
      region: orderData.region,
    });
...
@Module({
  imports: [TelemetryModule, MetricsModule, OrdersModule],
  providers: [
    {
...
**HTTP Metrics:**
- `http.requests.total` (Counter) - Total count of HTTP requests with method, route, and status attributes
- `http.request.duration` (Histogram) - Distribution of HTTP request latencies in milliseconds
- `http.requests.active` (UpDownCounter) - Current number of active HTTP requests
...
- [Instrument your NestJS application with traces](https://signoz.io/docs/instrumentation/javascript/opentelemetry-nodejs/) to correlate metrics with traces for better observability.
