...
<Tabs>
<TabItem value="custom" label="Custom Instrumentation" default>
...
<Admonition type="info">
The synchronous `Gauge` instrument is experimental and requires the `otel_unstable` feature flag in `opentelemetry`. This feature was added in OpenTelemetry Rust SDK v0.22.0. If you prefer stable APIs only, use `ObservableGauge` from the Observable Instruments tab instead.
</Admonition>

</TabItem>

<TabItem value="observable" label="Observable Instruments">
...
<TabItem value="http-server" label="HTTP Server Example">
