...
A metric is not just "CPU usage." A metric is CPU usage *with labels attached*. // explain labels before and breakdown the code below as people don't read comments inside code blocks
...
With 2 servers and 1,000 users, you now have 2 × 1,000 = **2,000 time series** from a single metric. // user might not know what is a time series, handle that with a tooltip
...
This is why observability vendors charge by cardinality as high cardinality genuinely costs more in infrastructure (more memory for indexes, more CPU for queries, more storage for data).
...
Here is the chain of events: // abrupt jump, chain of events for what? we need better transition.
...
Apart from compression issues, doing analytical queries in row stores also suffer from high I/O. To answer "What is the average latency for user X?" the database must read entire rows, including columns you do not need, just to extract the latency values. This wastes enormous amounts of I/O.
...
*   **Better compression:** Values of the same type are stored together. A column of `status` codes (200, 200, 200, 500, 200...) compresses beautifully because there are patterns. Even high-cardinality columns like `user_id` benefit from dictionary encoding. // explain dictionary encoding in a tool tip.
...
<SamplingAggregation /> // change "fidelity" to a simpler word

**Aggregation** is best for metrics. Instead of tracking latency per user, track histogram buckets. Instead of counting requests per `container_id`, count per service. You lose individual data points but keep statistical accuracy. // track histogram bucket is not clear
...
// add get started with signoz with smooth transition
