...
High cardinality is the silent killer of monitoring systems. It crashes Prometheus servers, inflates cloud bills, and turns simple queries into minute-long waits. This guide explains why it happens and what you can do about it.
...
When you count total users, there is exactly **1 group**. When you group by region, there are **4 groups**. When you group by user ID, there are **40 groups**, one for each unique user.
...
To understand cardinality explosion, you first need to understand how <Tooltip text="metrics" content="A metric is a measurement collected over time, such as CPU usage, request count, or error rate." link="https://signoz.io/blog/opentelemetry-metrics-with-examples/" /> &nbsp; work in time-series databases like Prometheus. // remove double underline from tooltip and fix the margin/padding

A metric is not just "CPU usage." A metric is CPU usage *with labels attached*. // explain labels before and breakdown the code below
...
<div className="bg-zinc-800 text-zinc-100 rounded-lg p-6 my-8 font-mono text-sm overflow-x-auto">
  ...
  <div>
    ...
    <span className="text-blue-400">cpu_usage</span>{`{`}<span className="text-green-400">host</span>=<span className="text-yellow-300">"server-1"</span>, <span className="text-red-400">user_id</span>=<span className="text-yellow-300">"789"</span>{`}`}<br/>
...
Each unique combination of label values creates a new time series. Add more labels, and they *multiply*. The formula is simple:
...
A single metric with four labels can explode into billions of time series. Try it below—click each label to activate it and watch the count grow.

<CardinalityExplosion />

This is not a theoretical problem. Teams hit this wall in production every week. One innocent-looking label like `user_id` can multiply your series count by a million—and potentially crash your entire monitoring system.
...
When you query by a high-cardinality field like `user_id=abc123`, the index still works—but it has millions of entries to manage. Finding your specific user means navigating a massive index structure, which takes more time and memory.
...
This is why observability vendors charge by cardinality. High cardinality genuinely costs more in infrastructure—more memory for indexes, more CPU for queries, more storage for data.
...
3. Each time series requires ~3-4 KB of memory just for its index entry
...
Row stores handle high cardinality better than TSDBs for inserts—there is no massive index to update. But they have a different problem: **entropy**.

When columns with very different data types and patterns are stored together (timestamps next to random UUIDs next to short status codes), the data stream becomes chaotic. This chaos—entropy—makes <Tooltip text="compression" content="Storing data in less space by finding patterns. High-cardinality columns have no patterns, so they don't compress well." /> nearly impossible. A billion random `user_id` values cannot be compressed efficiently because there are no patterns to exploit.

Analytical queries also suffer. To answer "What is the average latency for user X?" the database must read entire rows—including columns you do not need—just to extract the latency values. This wastes enormous amounts of I/O.
...
<div className="bg-zinc-800 rounded-lg p-6 my-8 font-mono text-sm overflow-x-auto">
  ...
  <div className="flex gap-6 text-zinc-300 justify-center">
...
*   **No index explosion:** There is no giant in-memory index mapping labels to series. Adding a new unique `user_id` is just appending a value to a column—not restructuring an index tree.
*   **Better compression:** Values of the same type are stored together. A column of `status` codes (200, 200, 200, 500, 200...) compresses beautifully because there are patterns. Even high-cardinality columns like `user_id` benefit from dictionary encoding.
...
**Aggregation** is best for metrics. Instead of tracking latency per user, track histogram buckets. Instead of counting requests per `container_id`, count per service. You lose individual data points but keep statistical accuracy.
...
These strategies are not mutually exclusive. Use aggregated metrics for dashboards and alerting, sampled traces for debugging, and full logs for compliance. Match the strategy to the use case.

## The Takeaway

High cardinality is a fundamental constraint, not a bug to fix. Every unique value has a cost. The question is whether that cost is worth it for your use case.
