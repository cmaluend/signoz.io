...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      title: 'Error',
      description: 'An error occurred while loading the comparison.',
    }
  }
}
...
export default async function Page({ params }: { params: { slug: string[] } }) {
  if (!params.slug || params.slug.length === 0) {
    return <div className="min-h-screen">Redirecting to comparisons index...</div>
  }

  const path = params.slug.join('/')
  console.log(`Fetching comparison content for path: ${path}`)
  ...
  const structuredData = generateStructuredData('comparisons', content)
  ...
  const mainContent: CoreContent<MDXContent> = {
    ...
    slug: path,
    path: content.path || `/comparisons/${path}`,
    type: 'Comparison',
    readingTime: readingTimeData,
    filePath: `/comparisons/${path}`,
